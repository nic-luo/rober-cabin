...
public class DataUpdater extends AbstractUpdater implements BeanSelfAware  {
    ...
    private <T> void fillSerialNos(List<T> objects) {
        ...
        for (Field field : fields) {
            ...
            GeneratedValue gv = field.getAnnotation(GeneratedValue.class);
            String generatorId = StringKit.isBlank(gv.generator()) ?
                    firstObject.getClass().getName() + "." + field.getName() :
                    gv.generator();
            SerialNoGenerator<String> serialNoGenerator = serialNoGeneratorFinder.find(generatorId);
            String[] genSerialNoArray = serialNoGenerator.nextBatch(generatorId, genCount);
            ArrayDeque<String> serialNoQueue = new ArrayDeque<>();
            serialNoQueue.addAll(ListKit.listOf(genSerialNoArray));
            ...
            for(int i=0;i<objects.size();i++){
                ...
                Object fieldValue = BeanKit.getPropertyValue(object, field.getName());
                if (fieldValue != null && StringKit.isNotBlank(fieldValue.toString())) continue;
                BeanKit.setPropertyValue(object, field.getName(),serialNoQueue.remove());
            }
        }
    }
