...
public abstract class SQLKit {
    ...
    private static Pattern SQL_SELECT_PATTERN = Pattern.compile("^\\s*\\(.*SELECT\\s+.+\\)\\s*$",Pattern.CASE_INSENSITIVE);
     ...
     * @return boolean
     */
    public static boolean isSelect(String column){
        return SQL_SELECT_PATTERN.matcher(column).find();
    }
